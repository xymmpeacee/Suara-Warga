...
<head>
    ...
    <style>
        ...
        .border-pending::before { background-color: #f97316; }
        .border-diproses::before { background-color: #3b82f6; }
        .border-selesai::before { background-color: #22c55e; }
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="openEditModal({{ $c->id }})" class="w-8 h-8 rounded-full bg-[#0052cc] text-white flex items-center justify-center hover:bg-[#003d99] shadow-sm transition-colors">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/></svg>
                                </button>

                                <form method="POST" action="{{ route('admin.dashboard.destroy', $c->id) }}"
                                    onsubmit="return confirm('Yakin ingin menghapus aduan {{ $c->ticket_code }}? Tindakan ini tidak dapat dibatalkan.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-full bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-100 shadow-sm transition-colors border border-red-200">
