...
<head>
    ...
    <script>
        // Harus didefinisikan sebelum Alpine.js load
        document.addEventListener('alpine:init', () => {
            Alpine.data('priorityDropdown', () => ({
                open: false,
                selected: @json(old('priority', '')),
                get label() {
                    const map = {
                        rendah: 'Rendah',
                        sedang: 'Sedang',
                    ...
                    return this.selected ? (map[this.selected] || this.selected) : 'Pilih prioritas';
                },
                options: [{
                        value: 'rendah',
                        label: 'Rendah'
...
<body class="font-sans antialiased text-gray-900" style="background-color: #F0F2F5;">
                ...
                <div class="space-y-8">
                    ...
                    <div class="grid sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div x-data="priorityDropdown" class="relative">
                                <input type="hidden" name="priority" :value="selected">
                                <button type="button" @click.stop="open = !open" @click.outside="open = false"
                                    class="flex items-center justify-between w-full rounded-xl border border-gray-300 bg-white shadow-sm text-sm py-3 pl-4 pr-4 text-gray-700 hover:border-primary/50 transition-colors">
                                    <span :class="selected ? 'text-gray-900' : 'text-gray-400'" x-text="label"></span>
                                    <svg class="w-4 h-4 text-gray-400 shrink-0 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                ...
                                <div x-show="open"
                                    ...
                                    <template x-for="opt in options" :key="opt.value">
                                        <button type="button" @click="select(opt)"
                                            class="w-full text-left px-4 py-2.5 text-sm hover:bg-primary/5 hover:text-primary transition-colors"
    ...
    <script>
        ...
        const defaultLat = @json(old('latitude', -6.9175));
        const defaultLng = @json(old('longitude', 107.6191));
        ...
        // Pastikan map render dengan benar setelah DOM siap
        ...
        function updateLocation(lat, lng, doReverseGeocode = true) {
            ...
            if (doReverseGeocode) {
                fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1`, {
                        headers: {
                            'User-Agent': 'SuaraWarga/1.0 (suarawarga@gmail.com)',
        ...
        updateLocation(@json(old('latitude')), @json(old('longitude')), false);
        @endif
        ...
        document.getElementById('crop-confirm').addEventListener('click', () => {
            ...
            cropper.getCroppedCanvas({
                ...
                const croppedFile = new File([blob], 'foto-aduan.jpg', {
                    type: 'image/jpeg'
                });
